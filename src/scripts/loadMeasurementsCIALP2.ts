...
if (!CLIENT_ID) {
  ...
  console.error('Uso: npm run load:measurements:cialp2 <clientId>');
  console.error('Ejemplo: npm run load:measurements:cialp2 6924acfdfe0e9f6c29d35966');
  process.exit(1);
}
...
const EXCEL_FILE_PATH = 'CIAL P2_TABLA RESUMEN.xlsx';

// Configuracion de hojas a procesar
const SHEET_CONFIG = [
  { routeSheet: 'CIAL P2_Ruta_Cutter', faultSheet: 'Fallas cutter', opening: 'Cutter' },
  { routeSheet: 'CIAL P2_Ruta_Cámara 1', faultSheet: 'Fallas camara 1', opening: 'Cámara 1' },
  { routeSheet: 'CIAL P2_Ruta_Cámara 2', faultSheet: 'Fallas camara 2', opening: 'Cámara 2' },
  { routeSheet: 'CIAL P2_Ruta_Cámara 3', faultSheet: 'Fallas camara 3', opening: 'Cámara 3' },
  { routeSheet: 'CIAL P2_Ruta_Cámara 4', faultSheet: 'Fallas camara 4', opening: 'Cámara 4' },
];
...
const FAULT_TYPE_MAPPING: Record<string, string> = {
  ...
  'TRANSM DE POTENCIA': 'powerTransmission',
  'SENSOR SIN SEÑAL': 'sensorNoSignal',
  'SENSOR SIN SENAL': 'sensorNoSignal',
};

// Campos de tipos de falla que no se mapean (se ignoran)
const IGNORED_FAULT_TYPES: string[] = [];
...
// En este Excel las fechas son el primer dia del mes en UTC (2023-01-01 = Enero 2023)
// Usamos getUTC* para evitar problemas de timezone
...
// En este Excel: fila 3 tiene fechas, datos de falla empiezan en fila 4
const readFaultData = (
  worksheet: ExcelJS.Worksheet,
): Map<string, FaultData> => {
  const dataMap = new Map<string, FaultData>();

  // La fila 3 contiene las fechas (columnas 3 en adelante)
  ...
  // Leer tipos de falla (filas 4 en adelante)
...
const loadMeasurementsFromExcel = async (): Promise<void> => {
  try {
    ...
    console.log('CARGA DE MEDICIONES - CIAL P2');
    console.log('='.repeat(60));
    ...
    // Procesar cada configuracion de hojas
    let stepNum = 2;
    for (const config of SHEET_CONFIG) {
      console.log(`\n${stepNum}. Procesando ${config.opening}...`);

      const routeSheet = workbook.getWorksheet(config.routeSheet);
      const faultSheet = workbook.getWorksheet(config.faultSheet);

      if (!routeSheet || !faultSheet) {
        console.error(`   Error: No se encontraron las hojas para ${config.opening}`);
        console.error(`   Ruta esperada: "${config.routeSheet}"`);
        console.error(`   Fallas esperada: "${config.faultSheet}"`);
        continue;
      }

      const routeData = readRouteData(routeSheet);
      const faultData = readFaultData(faultSheet);
      console.log(`   Periodos encontrados: ${routeData.size}`);

      routeData.forEach((periodData, key) => {
        const faults = faultData.get(key) || {};
        measurements.push(
          createMeasurementData(periodData, faults, config.opening, clientObjectId)
        );
      });
    ...
    console.log(`\n${stepNum}. Total de mediciones a procesar: ${measurements.length}`);

    // Mostrar resumen de datos
    console.log(`\n${stepNum + 1}. Resumen de datos a cargar:`);
    console.log('-'.repeat(80));

    let currentOpening = '';
    measurements.forEach((m, index) => {
      if (m.opening !== currentOpening) {
        currentOpening = m.opening;
        console.log(`\n--- ${currentOpening} ---`);
      }
      const ym = `${m.year}-${String(m.month).padStart(2, '0')}`;
      console.log(
        `${index + 1}. ${ym} | B:${m.good} O:${m.observation} I:${m.unsatisfactory} P:${m.danger} NM:${m.unmeasured}`
      );
    });

    // Conectar a la base de datos
    console.log(`\n${stepNum + 2}. Conectando a la base de datos...`);
    await mongoose.connect(MONGO_URI);
    console.log('   Conexion exitosa a MongoDB');

    // Realizar upsert para cada medicion
    console.log(`\n${stepNum + 3}. Ejecutando upsert de mediciones...`);
    let insertedCount = 0;
    let updatedCount = 0;
